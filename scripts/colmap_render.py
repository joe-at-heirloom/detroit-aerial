...
def _render_coarse(meta, cams, imgs, ims, mpp):
    """The whole block rendered flat at `mpp`, for measuring it against a reference."""
    ...
    for name, img in imgs.items():
        r = render_frame(name, img, cams[img['cam']], ims[name], 0.0, E0, N0, minE, maxN, W, H, mpp, crop=0.95)
    ...
    return comp, bbox, minE, maxN, W, H


def _fit_similarity(A, B):
    """Robust 2D similarity A -> B (IRLS on a Umeyama fit). Returns sc, R, t, keep, residuals."""
    ...
    return sc, R2, t2, keep, res


def _apply_similarity(imgs, sc, R2, t2, E0, N0):
    """Fold X' = sc*R2*X + t2 (in the E-E0, N-N0 frame) into every camera and the sparse points."""
    Q = np.eye(3); Q[:2, :2] = R2
...
def refine_against(work, meta, cams, imgs, ref, mpp=5.0, search_m=600.0, log=print, max_pass=3):
    ...
    arterial coarse field (6 km windows, wide-road ridge), fit a 2D similarity to
    those offsets with robust rejection, and fold it into the camera alignment.
    Four parameters from ~30 windows: well determined, and it cannot bend anything.

    It iterates, because one pass is not enough when the heading is far off: the
    windows at the block's ends sit outside the search and the fit is made from
    the middle only. 1956 (33 km long, 1.7 deg off) locked 9 windows of 29 at
    +/-600 m and fitted a similarity with a 263 m residual. So: if fewer than half
    the windows lock, the search widens to 1500 m (a mile grid cannot alias inside
    that); a fit whose residual is over 60 m is not applied; and after applying
    a fit the block is re-rendered and measured again until the correction is
    small."""
    import gridval
    ...
    while it < max_pass:
        ...
        pts = gridval.coarse_field(t, search_m=search, log=log)
        n_win = getattr(gridval.coarse_field, 'last_n_windows', 0) or max(len(pts), 1)
        if len(pts) < max(8, 0.5 * n_win) and not widened:
            log(f"  refine-against {ref}: {len(pts)} of {n_win} windows locked at +/-{search:.0f} m; "
